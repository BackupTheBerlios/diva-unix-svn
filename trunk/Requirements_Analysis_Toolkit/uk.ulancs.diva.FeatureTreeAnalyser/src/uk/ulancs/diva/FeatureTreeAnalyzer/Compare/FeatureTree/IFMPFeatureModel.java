...
public class IFMPFeatureModel extends AbstractFeatureModel {
	...
	public IFMPFeatureModel(FmpExternalLoader FileLoader, String TreeName) {
	...
	public void IndexNodes() {
		...
		FetchFMPNodes(model.getChildren(),model.getName(),model, null, 0, false, false);
	}
	
	public void FetchFMPNodes(EList l,String owner,Node root, FNode CurrentParent, int CurrentDepth, Boolean IsOr, Boolean IsXor){
				...
				if(node!=null){
					...
					if (nodeName!=null) {
						if (nodeName.contains("group")) {
							...
							FetchFMPNodes(node.getChildren(),nodeName,root,CurrentParent, CurrentDepth, Or, Xor);
						}
						else {
							...
							if(nodeId!=null && nodeDesc!=null){
								...
								FetchFMPNodes(node.getChildren(),nodeName,root,NewNode, CurrentDepth+1, IsOr, IsXor);
							}
						}
					}
				}
