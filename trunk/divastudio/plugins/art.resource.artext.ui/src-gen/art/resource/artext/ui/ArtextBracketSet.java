...
public class ArtextBracketSet {
	...
	private java.util.ArrayList<art.resource.artext.IArtextBracketPair> bracketPairs;
	 ...
	 */
	private class BracketPair implements art.resource.artext.IArtextBracketPair {
	 ...
	 */
	private class ClosingListener implements art.resource.artext.ui.IArtextBracketHandler, org.eclipse.swt.events.VerifyListener, org.eclipse.swt.events.ModifyListener, org.eclipse.swt.custom.VerifyKeyListener {
		...
		public void verifyText(org.eclipse.swt.events.VerifyEvent e) {
			...
			if (caret > 0 && caret < textWidget.getCharCount()) {
				art.resource.artext.IArtextBracketPair bracketPair = getBracketPair(textWidget.getTextRange(caret - 1, 1), textWidget.getTextRange(caret, 1));
	 ...
	 */
	public ArtextBracketSet(art.resource.artext.ui.ArtextEditor editor, org.eclipse.jface.text.source.ISourceViewer sourceViewer) {
		languageID = new art.resource.artext.mopp.ArtextMetaInformation().getSyntaxName();
		this.bracketPairs = new java.util.ArrayList<art.resource.artext.IArtextBracketPair>();
	 ...
	 */
	public art.resource.artext.IArtextBracketPair getBracketPair(String opening, String closing) {
		for (art.resource.artext.IArtextBracketPair bracketPair : bracketPairs) {
	...
	public art.resource.artext.IArtextBracketPair getBracketPair(int index) {
		try {
	 ...
	 */
	public boolean setClosingEnabledInside(art.resource.artext.IArtextBracketPair bracketPair, boolean closingEnabledInside) {
	...
	public boolean resetBrackets() {
		String bracketPairs = preferenceStore.getString(languageID + art.resource.artext.ui.ArtextPreferenceConstants.EDITOR_BRACKETS_SUFFIX);
	 ...
	 */
	public art.resource.artext.IArtextBracketPair remove(String opening, String closing) {
		for (art.resource.artext.IArtextBracketPair bracketPair : bracketPairs) {
			if (bracketPair.getOpeningBracket().equals(opening) && bracketPair.getClosingBracket().equals(closing)) {
	...
	public boolean setBrackets(String bracketSet) {
		...
		bracketPairs = new java.util.ArrayList<art.resource.artext.IArtextBracketPair>();
	...
	public void matchingBrackets() {
		...
		if (position != -1 && position != document.getLength()) {
			positionHelper.addPosition(document, art.resource.artext.ui.ArtextPositionCategory.BRACKET.toString(), position, 1);
			positionHelper.addPosition(document, art.resource.artext.ui.ArtextPositionCategory.BRACKET.toString(), caretOffset - 1, 1);
		}
	}
