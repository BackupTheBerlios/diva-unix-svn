 ...
 */
public class ArtextOccurrence {
	
	public final static String OCCURRENCE_ANNOTATION_ID = "art.resource.artext.ui.occurences";
	public final static String DECLARATION_ANNOTATION_ID = "art.resource.artext.ui.occurences.declaration";
	...
	private art.resource.artext.ui.ArtextTokenScanner tokenScanner;
	...
	private art.resource.artext.IArtextTextResource textResource;
	 ...
	 */
	public ArtextOccurrence(art.resource.artext.IArtextTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceViewer, art.resource.artext.ui.ArtextTokenScanner tokenScanner) {
		this.textResource = textResource;
		this.projectionViewer = sourceViewer;
		
		quotedTokenArray = new java.util.ArrayList<String>();
		String[] tokenNames = new art.resource.artext.mopp.ArtextMetaInformation().getTokenNames();
	 ...
	 */
	public void handleOccurrenceHighlighting(art.resource.artext.ui.ArtextBracketSet bracketSet) {
		...
		if (resource.equals(textResource)) {
			...
			while (!token.isEOF()) {
				...
				if (text.equals(tokenText)) {
					...
					addAnnotation(document, art.resource.artext.ui.ArtextPositionCategory.DEFINTION, text);
					break;
				}
				token = tokenScanner.nextToken();
			}
			...
			if (text != null && text.equals(tokenText) && tokenScanner.getTokenOffset() != defPosition) {
				...
				if (occEO != null) {
					if ((isNull && elementsAtDefinition.contains(occEO)) || !isNull && definitionElement.equals(occEO)) {
						addAnnotation(document, art.resource.artext.ui.ArtextPositionCategory.PROXY, text);
					}
				}
			}
			token = tokenScanner.nextToken();
	...
	private void addAnnotation(org.eclipse.jface.text.IDocument document, art.resource.artext.ui.ArtextPositionCategory type, String text) {
		int tokenOffset = tokenScanner.getTokenOffset();
		int tokenLength = tokenScanner.getTokenLength();
		...
		if (type == art.resource.artext.ui.ArtextPositionCategory.DEFINTION) {
	...
	private void removeAnnotations() {
		removeAnnotations(art.resource.artext.ui.ArtextOccurrence.OCCURRENCE_ANNOTATION_ID);
		removeAnnotations(art.resource.artext.ui.ArtextOccurrence.DECLARATION_ANNOTATION_ID);
	}
