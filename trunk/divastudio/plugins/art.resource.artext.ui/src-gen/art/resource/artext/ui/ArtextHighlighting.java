 ...
 */
public class ArtextHighlighting implements org.eclipse.jface.viewers.ISelectionProvider, org.eclipse.jface.viewers.ISelectionChangedListener {
	...
	private final static art.resource.artext.ui.ArtextPositionHelper positionHelper = new art.resource.artext.ui.ArtextPositionHelper();
	private boolean isHighlightBrackets = true;
	private art.resource.artext.ui.ArtextTokenScanner scanner;
	...
	private art.resource.artext.ui.ArtextOccurrence occurrence;
	private art.resource.artext.ui.ArtextBracketSet bracketSet;
	 ...
	 */
	private final class UpdateHighlightingListener implements org.eclipse.swt.events.KeyListener, org.eclipse.swt.events.VerifyListener, org.eclipse.swt.events.MouseListener, art.resource.artext.IArtextBackgroundParsingListener {
	 ...
	 * @param editor
	 */
	public ArtextHighlighting(art.resource.artext.IArtextTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceviewer, art.resource.artext.ui.ArtextColorManager colorManager, art.resource.artext.ui.ArtextEditor editor) {
		this.display = org.eclipse.swt.widgets.Display.getCurrent();
		sourceviewer.getSelectionProvider();
		preferenceStore = art.resource.artext.ui.ArtextUIPlugin.getDefault().getPreferenceStore();
		textWidget = sourceviewer.getTextWidget();
		projectionViewer = sourceviewer;
		scanner = new art.resource.artext.ui.ArtextTokenScanner(textResource, colorManager);
		occurrence = new art.resource.artext.ui.ArtextOccurrence(textResource, sourceviewer, scanner);
		bracketSet = new art.resource.artext.ui.ArtextBracketSet(editor, sourceviewer);
		this.colorManager = colorManager;
		isHighlightBrackets = preferenceStore.getBoolean(art.resource.artext.ui.ArtextPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX);
		bracketColor = colorManager.getColor(org.eclipse.jface.preference.PreferenceConverter.getColor(preferenceStore, art.resource.artext.ui.ArtextPreferenceConstants.EDITOR_MATCHING_BRACKETS_COLOR));
		black = colorManager.getColor(new org.eclipse.swt.graphics.RGB(0, 0, 0));
		
		addListeners(editor);
	}
	
	private void addListeners(art.resource.artext.ui.ArtextEditor editor) {
	...
	private void setBracketHighlighting(org.eclipse.jface.text.IDocument document) {
		...
		org.eclipse.jface.text.Position[] positions = positionHelper.getPositions(document, art.resource.artext.ui.ArtextPositionCategory.BRACKET.toString());
	...
	private void removeHighlighting() {
		...
		removeHighlightingCategory(document, art.resource.artext.ui.ArtextPositionCategory.BRACKET.toString());
	}
	...
	private void removeHighlightingCategory(org.eclipse.jface.text.IDocument document, String category) {
		...
		if (category.equals(art.resource.artext.ui.ArtextPositionCategory.BRACKET.toString())) {
		...
		display.syncExec(new Runnable() {
			public void run() {
				...
				if (selectedEObject != null) {
					setSelection(new art.resource.artext.ui.ArtextEObjectSelection(selectedEObject, false));
				}
			}
		});
	}
	...
	private void handleContentOutlineSelection(org.eclipse.jface.viewers.ISelection selection) {
		if (!selection.isEmpty()) {
			...
			if (selectedElement instanceof org.eclipse.emf.ecore.EObject) {
				...
				if (resource instanceof art.resource.artext.IArtextTextResource) {
