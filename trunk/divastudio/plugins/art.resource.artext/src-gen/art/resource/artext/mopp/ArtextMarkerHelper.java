...
public class ArtextMarkerHelper {
	 ...
	 */
	public static final String MARKER_TYPE = art.resource.artext.mopp.ArtextPlugin.PLUGIN_ID + ".problem";
	/**
	 ...
	 */
	public static void mark(org.eclipse.emf.ecore.resource.Resource resource, final art.resource.artext.IArtextTextDiagnostic diagnostic) {
	...
	private static void createMarkerFromDiagnostic(org.eclipse.core.resources.IFile file, final art.resource.artext.IArtextTextDiagnostic diagnostic) {
		try {
			...
			if (diagnostic.getProblem().getType() == art.resource.artext.ArtextEProblemType.ERROR) {
			...
			java.util.Collection<art.resource.artext.IArtextQuickFix> quickFixes = textDiagnostic.getProblem().getQuickFixes();
			...
			if (!sourceIDs.isEmpty()) {
				marker.setAttribute(org.eclipse.core.resources.IMarker.SOURCE_ID, art.resource.artext.util.ArtextStringUtil.explode(sourceIDs, "|"));
			}
		} catch (org.eclipse.core.runtime.CoreException ce) {
			if (ce.getMessage().matches("Marker.*not found.")) {
				// ignore
			} else {
				art.resource.artext.mopp.ArtextPlugin.logError("Error marking resource:", ce);
			}
		}
	}
