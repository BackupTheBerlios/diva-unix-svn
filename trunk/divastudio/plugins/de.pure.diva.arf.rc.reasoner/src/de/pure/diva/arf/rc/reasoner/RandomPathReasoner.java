...
public class RandomPathReasoner extends RandomReasoner implements Reasoner, BundleActivator {

  public static final String NAME = "Random Path Reasoner";
  ...
  protected Result solve(ModelHandle model, InputHandle input, QuestionHandle question) {
    ...
    if (ARF.QUESTIONS.ARE_THERE_VALID_CONFIGURATIONS.equals(question.getQuestion()) == true) {
      RandomConfigurationSolver solver = new RandomConfigurationSolver(question.getParameters().TIME_CONSUMPTION, 1, RandomConfigurationSolver.NO, false);
      result = solver.exist(model, input);
    }
    else if (ARF.QUESTIONS.GET_ONE_VALID_CONFIGURATION.equals(question.getQuestion()) == true) {
      RandomConfigurationSolver solver = new RandomConfigurationSolver(question.getParameters().TIME_CONSUMPTION, 1, RandomConfigurationSolver.ALL, false);
  ...
  public ConfigurationResult solve(ModelHandle model, InputHandle input) {
    RandomConfigurationSolver solver = new RandomConfigurationSolver(RandomConfigurationSolver.NOTIMEOUT, 1, RandomConfigurationSolver.ALL, false);
