...
public class DiVAMetaModelAreThereConfigurations extends DiVAMetaModelAction {
  ...
  protected void run(IProgressMonitor monitor, Resource vmodel) throws Exception {
    QuestionHandle question = new arf.question.QuestionHandle(ARF.QUESTIONS.ARE_THERE_VALID_CONFIGURATIONS);
    ReasonerHandle reasoner = getReasoner(vmodel, "Are there valid Configurations?", question);
    if (reasoner != null) {
      ...
      for (Resource resource : contexts.keySet()) {
        ...
        BooleanResult result = (BooleanResult) arf.ask(question);
      ...
      ResultRunnable runnable = new ResultRunnable(results, "Are there valid Configurations?", logging);
