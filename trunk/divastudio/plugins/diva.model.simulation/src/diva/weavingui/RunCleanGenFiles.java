...
public class RunCleanGenFiles implements IObjectActionDelegate, Runnable {

	protected StructuredSelection currentSelection;
    protected IFile file;
	
	public RunCleanGenFiles() {
		super();
	}
	...
	public void run() {
		...
		console.println(new InfoMessage("Cleaning generated files for : " + file.getLocation().toOSString() + "..."));
		...
		try {
			...
			IResource configs_gen_folder = diva_folder.findMember("gen_configurations", true);
			
			if (aspect_gen_folder != null && aspect_gen_folder.exists()) aspect_gen_folder.delete(true, null);
			if (configs_gen_folder != null && configs_gen_folder.exists()) configs_gen_folder.delete(true, null);
			
			diva_folder.getFolder(Path.fromPortableString("gen_aspects")).create(true, false, null);
			diva_folder.getFolder(Path.fromPortableString("gen_configurations")).create(true, false, null);
